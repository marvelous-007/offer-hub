...
impl StatContract {
    ...
    pub fn get_contract_stats(env: Env) -> Result<ContractStats, Error> {
        let user_registry_contract_id = env.storage()
            .instance()
            .get::<_, Address>(&DataKey::UserRegistryContract)
            .ok_or(Error::NotInitialized)?;

        let rating_contract_id = env.storage()
            .instance()
            .get::<_, Address>(&DataKey::RatingContract)
            .ok_or(Error::NotInitialized)?;

        let escrow_contract_id = env.storage()
            .instance()
            .get::<_, Address>(&DataKey::EscrowContract)
            .ok_or(Error::NotInitialized)?;
            

        let dispute_contract_id = env.storage()
            .instance()
            .get::<_, Address>(&DataKey::DisputeContract)
            .ok_or(Error::NotInitialized)?;

        let fee_manager_id = env.storage()
            .instance()
            .get::<_, Address>(&DataKey::FeeManagerContract)
            .ok_or(Error::NotInitialized)?;
            

        let user_registry_client = user_registry_contract::ContractClient::new(&env, &user_registry_contract_id);
        ...
        let dispute_client = dispute_contract::DisputeResolutionContractClient::new(&env, &dispute_contract_id);
        let total_disputes = dispute_client.get_total_disputes();

        let dispute_client = fee_manager_contract::FeeManagerContractClient::new(&env, &fee_manager_id);
        let total_fees_collected = dispute_client.get_total_fees();
        
        Ok(ContractStats { total_users, total_transactions, total_ratings, total_disputes, total_fees_collected })
    }
}