...
use super::*;
use crate::utils::{StorageOptimizer, ValidationHelpers};
...
fn test_validation_helpers() {
    ...
    let long_text = String::from_str(
        &env,
        "This is a very long text that exceeds the maximum allowed length for testing purposes",
    );
    let valid_text = String::from_str(&env, "Valid text");

    assert!(!ValidationHelpers::validate_string_length(
        &short_text,
        3,
        10
    ));
    ...
    assert!(ValidationHelpers::validate_publication_type(
        &env,
        &service_type
    ));
    assert!(ValidationHelpers::validate_publication_type(
...
fn test_optimized_publication_contract() {
    ...
    assert!(ValidationHelpers::validate_title(&String::from_str(
        &env,
        "Valid Title"
    )));
    assert!(!ValidationHelpers::validate_title(&String::from_str(
    ...
    assert!(ValidationHelpers::validate_category(&String::from_str(
        &env,
        "Technology"
    )));
    assert!(!ValidationHelpers::validate_category(&String::from_str(
