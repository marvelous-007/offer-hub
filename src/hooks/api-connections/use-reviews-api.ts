...
import { UpdateReviewDTO } from "@/types/reviews.types";

type UseReviewsApiReturn = {
  fetchUserReviews: (userId: string) => Promise<Review[]>;
  createReview: (input: CreateReviewDTO) => Promise<Review>;
  updateReview: (input: UpdateReviewDTO) => Promise<Review>;
  deleteReview: (reviewId: string) => Promise<void>;
  useUserReviews: (userId: string) => {
    data: Review[] | undefined;
    isLoading: boolean;
    error?: string;
  };
  useCreateReview: () => {
    mutate: (input: CreateReviewDTO) => Promise<Review>;
    isLoading: boolean;
    error?: string;
  };
...
export function useReviewsApi(): UseReviewsApiReturn {
  const [userReviewsCache, setUserReviewsCache] = useState<
    Record<string, { data: Review[]; timestamp: number }>
  >({});
  ...
  const fetchUserReviews = useCallback(
    async (userId: string): Promise<Review[]> => {
      ...
      try {
        ...
        const data: ReviewsFetchResponse | ReviewErrorResponse = await response
          .json()
          .catch(() => ({
            success: false,
  ...
  const createReview = useCallback(
    async (input: CreateReviewDTO): Promise<Review> => {
      ...
      try {
        ...
        const data: ReviewCreateResponse | ReviewErrorResponse = await response
          .json()
          .catch(() => ({
            success: false,
            message: `HTTP ${response.status}: ${response.statusText}`,
  ...
  const useUserReviews = (userId: string) => {
    ...
    const isCacheValid =
      cached && Date.now() - cached.timestamp < 5 * 60 * 1000; // 5 minutes
  ...
  const updateReview = useCallback(
    async (input: UpdateReviewDTO): Promise<Review> => {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/reviews/${input.id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(input),
        });

        const data: ReviewUpdateResponse | ReviewErrorResponse = await response
          .json()
          .catch(() => ({
            success: false,
            message: `HTTP ${response.status}: ${response.statusText}`,
        ...
        if (data.success && "data" in data) {
          ...
          if (data.data.to_id) {
            setUserReviewsCache((prev) => {
              const updated = { ...prev };
              delete updated[data.data.to_id];
              return updated;
            });
          }
          return data.data;
  ...
  const deleteReview = useCallback(async (reviewId: string): Promise<void> => {
    ...
    try {
      ...
      const data: ReviewDeleteResponse | ReviewErrorResponse = await response
        .json()
        .catch(() => ({
          success: false,
          message: `HTTP ${response.status}: ${response.statusText}`,
      ...
      // No need to invalidate specific user caches here as the caller will handle it
      // based on the user IDs in the review data they already have
      return;
    } catch (err) {
      const message =
