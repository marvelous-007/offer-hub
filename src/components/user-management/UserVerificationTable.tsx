...
import { Check, ChevronDown, MoreHorizontal, Search, Calendar as CalendarIcon } from "lucide-react";
  ...
  <div className="flex flex-col md:flex-row flex-wrap items-start md:items-center justify-between gap-4 bg-white p-3 rounded border-b">
    ...
    <div className="flex flex-col sm:flex-row w-full md:w-auto flex-wrap items-start sm:items-center gap-3 md:gap-4">
      <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
        <Select
          value={userType}
          onValueChange={(value) => setUserType(value as "Freelancer" | "Customer")}
        >
      ...
      <table className="w-full border-collapse">
        <thead className="bg-[#F9FAFB]"> 
          <tr className="border-b border-[#f9fbfa] bg-[#F9FAFB] mb-2 p-3">
            ...
            <th className="text-left py-3 px-4 font-medium text-black text-sm">Identity Card</th>
            <th className="text-left py-3 px-4 font-medium text-black text-sm">Status</th>
            <th className="text-left py-3 px-4 font-medium text-black text-sm">Submission</th>
        ...
        <tbody className="bg-[#fffefe]">
          {filteredData.map((user) => (
            <tr key={user.id} className="hover:bg-gray-50">
              ...
              <td className="py-3 px-4 text-sm">
                <span className={`px-2 py-1 text-xs ${user.status === 'Approved' ? 'border border-[#B7EB8F] bg-[#F6FFED] text-[#52C41A]' : 'border border-[#FFC000] bg-[#FFF7E6] text-[#FA8C16]'}`}>
                  {user.status}
                </span>
              </td>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent>
                      ...
                      <DropdownMenuItem onClick={() => handleOverflowAction("approve", user.id)}>
                        Approve User
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => handleOverflowAction("reject", user.id)}>
                        Reject User
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
      ...
      <div key={user.id} className="bg-white rounded-lg shadow p-4 border border-gray-100">
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span>{new Date(user.submissionDate).toLocaleDateString("en-US", {
              month: "numeric", 
              day: "numeric",
              year: "2-digit",
            })}</span>
