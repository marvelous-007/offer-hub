...
export class PortfolioItemsService {
  constructor(
    @InjectRepository(PortfolioItem) private readonly repo: Repository<PortfolioItem>,
  ...
  async findById(portfolio_item_id: string): Promise<PortfolioItem> {
    const portfolioItem = await this.repo.findOne({ where: { portfolio_item_id }, relations: ['user'] });
    if (!portfolioItem) throw new NotFoundException(`Portfolio item with ID ${portfolio_item_id} not found.`);
    return portfolioItem;
  }

  async update(portfolio_item_id: string, dto: UpdatePortfolioItemDto): Promise<PortfolioItem> {
  ...
  async delete(portfolio_item_id: string): Promise<void> {
    ...
    if (result.affected === 0) throw new NotFoundException(`Portfolio item with ID ${portfolio_item_id} not found.`);
  }
}
