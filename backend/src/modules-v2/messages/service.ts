...
export class MessagesService {
  ...
  async create(dto: CreateMessageDto): Promise<Message> {
    const conversation = await this.conversationRepo.findOne({ where: { conversationId: dto.conversation_id } });
    if (!conversation) throw new NotFoundException(`Conversation with ID ${dto.conversation_id} not found.`);

    const sender = await this.userRepo.findOne({ where: { user_id: dto.sender_id } });
    if (!sender) throw new NotFoundException(`User with ID ${dto.sender_id} not found.`);
  ...
  async findById(message_id: string): Promise<Message> {
    const message = await this.repo.findOne({ where: { message_id }, relations: ['conversation', 'sender'] });
    if (!message) throw new NotFoundException(`Message with ID ${message_id} not found.`);
    return message;
  }
  ...
  async delete(message_id: string): Promise<void> {
    ...
    if (result.affected === 0) throw new NotFoundException(`Message with ID ${message_id} not found.`);
  }
}
