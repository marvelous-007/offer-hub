...
export class RatingsService {
  ...
  async findById(rating_id: string): Promise<Rating> {
    const rating = await this.repo.findOne({ where: { rating_id }, relations: ['fromUser', 'toUser', 'project'] });
    if (!rating) throw new NotFoundException(`Rating with ID ${rating_id} not found.`);
    return rating;
  }
  ...
  async delete(rating_id: string): Promise<void> {
    ...
    if (result.affected === 0) throw new NotFoundException(`Rating with ID ${rating_id} not found.`);
  }
}
