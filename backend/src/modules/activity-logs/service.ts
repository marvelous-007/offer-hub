...
export class ActivityLogsService {
    constructor(@InjectRepository(ActivityLogs) private readonly repo: Repository<ActivityLogs>) {}
    ...
    async findById(id: string) {
        ...
        if (!activityLog) throw new NotFoundException(`Activity Log with ID ${id} not found.`);
        return activityLog;
    }
    ...
    async delete(id: string) {
        ...
        if (result.affected === 0) throw new NotFoundException(`Activity Log with ID ${id} not found.`);
    }
}
