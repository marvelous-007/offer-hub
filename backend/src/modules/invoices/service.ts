...
export class InvoiceService {
  ...
  async generateInvoiceForTransaction(transactionId: string): Promise<string> {
    try {
      ...
      if (transaction.status !== TransactionStatus.COMPLETED) {
        this.logger.warn(
          `Cannot generate invoice for incomplete transaction: ${transactionId}`,
        );
        throw new Error(
          "Invoices can only be generated for completed transactions",
        );
      }
      ...
      if (transaction.invoice_path) {
        this.logger.log(
          `Transaction ${transactionId} already has a generated invoice`,
        );
      ...
      this.logger.log(
        `Invoice successfully generated for transaction ${transactionId}`,
      );
      return relativePath;
    } catch (error) {
      this.logger.error(
        `Error generating invoice for transaction ${transactionId}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async getInvoiceUrl(transactionId: string, userId: string): Promise<string> {
    try {
      ...
      if (
        transaction.fromUser.user_id !== userId &&
        transaction.toUser.user_id !== userId
      )
      ...
      if (transaction.invoice_path)
        return this.storageService.getFileUrl(transaction.invoice_path);
   ...
   */
  private async mapTransactionToInvoiceData(
    ...
    const isPayment =
      transaction.type === "payment" || transaction.type === "escrow_release";
