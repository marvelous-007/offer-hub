...
const BUSINESS = {
  ...
  FOOTER_TEXT:
    "Thank you for using OFFER-HUB - The decentralized freelancer platform",
...
export class PdfService {
      ...
      try {
        ...
        doc.on("end", async () => {
          try {
            ...
            this.logger.log(
              `Invoice generated and stored in MinIO: ${fileUrl}`,
            );
            resolve(fileUrl);
          } catch (error) {
            this.logger.error(
              `Error uploading invoice to MinIO: ${error.message}`,
            );
            reject(error);
          }
        });
  ...
  async getInvoiceUrl(
    ...
    try {
      ...
      return await this.storageService.getFileUrl(
        objectName,
        BUSINESS.URL_EXPIRATION_SECONDS,
      );
    } catch (error) {
  ...
  private addPartyInformation(
    ...
    if (invoiceData.freelancer.address)
      doc.text(invoiceData.freelancer.address);

    if (invoiceData.freelancer.walletAddress)
      doc.text(`Wallet: ${invoiceData.freelancer.walletAddress}`);
  ...
  private addInvoiceInformation(
    ...
    if (invoiceData.dueDate)
      doc.text("Due Date:", LAYOUT.INVOICE_INFO_LEFT_X, doc.y + 15);
    ...
    doc.text(
      ...
      LAYOUT.INVOICE_INFO_RIGHT_X,
      invoiceInfoY,
    );
    doc.text(
      this.formatDate(invoiceData.createdAt),
      LAYOUT.INVOICE_INFO_RIGHT_X,
      invoiceInfoY + 15,
    );
    ...
    if (invoiceData.dueDate) {
      doc.text(
        this.formatDate(invoiceData.dueDate),
        LAYOUT.INVOICE_INFO_RIGHT_X,
        doc.y,
      );
      doc.moveUp();
  ...
  private addItems(doc: typeof PDFDocument, invoiceData: InvoiceData): void {
    ...
    items.forEach((item, index) => {
      ...
      if (index % 2 === 0) {
        doc
          .fillColor(STYLE.TABLE_ROW_BG_COLOR)
          .rect(
            LAYOUT.TABLE_START_X,
            y,
            LAYOUT.TABLE_WIDTH,
            LAYOUT.TABLE_ROW_HEIGHT,
          )
          .fill();
      }
  ...
  private drawTableHeader(doc: typeof PDFDocument): void {
    ...
    // Header background
    doc
      .fillColor(STYLE.TABLE_HEADER_BG_COLOR)
      .rect(
        LAYOUT.TABLE_START_X,
        doc.y,
        LAYOUT.TABLE_WIDTH,
    ...
    doc.text(
      "Description",
      LAYOUT.TABLE_DESCRIPTION_X,
      doc.y + LAYOUT.TABLE_CELL_PADDING_Y,
      {
        width: LAYOUT.TABLE_DESCRIPTION_WIDTH,
      },
    );
  ...
  private addTotals(doc: typeof PDFDocument, invoiceData: InvoiceData): void {
    ...
    const y = doc.y;
    doc
      .strokeColor(STYLE.SEPARATOR_COLOR)
  ...
  private addFooter(doc: typeof PDFDocument, invoiceData: InvoiceData): void {
    ...
    doc
      ...
      .text(`Status: ${this.formatStatus(invoiceData.status)}`, {
        align: "center",
      });
    ...
    doc.moveDown();
    doc
      .fontSize(STYLE.FOOTER_FONT_SIZE)
      .text(BUSINESS.BLOCKCHAIN_NOTE(invoiceData.transactionHash), {
        align: "center",
      });
  }
